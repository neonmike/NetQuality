...
install_dependencies() {
    ...
    if [[ $is_dep -eq 0 || $is_nexttrace -eq 0 || $is_speedtest -eq 0 ]]; then
        ...
        if [[ $mode_yes -eq 0 ]]; then
            ...
            case "$choice" in
            y | Y | yes | Yes | YES)
                echo "Continue to execute script..."
                ;;
            n | N | no | No | NO)
                echo "Script exited."
                exit 0
        ...
        if [[ $is_dep -eq 0 ]]; then
            if [ "$(uname)" == "Darwin" ]; then
                ...
                case $ID in
                debian)
                    install_packages "apt" "apt-get install -y"
                    ;;
                ubuntu | linuxmint)
                    install_packages "apt" "apt-get install -y"
                    ;;
                rhel | centos | almalinux | rocky | anolis)
                    if
                    ...
                    ;;
                arch | manjaro)
                    install_packages "pacman" "pacman -S --noconfirm"
                    ;;
                alpine)
                    install_packages "apk" "apk add"
                    ;;
                fedora)
                    install_packages "dnf" "dnf install -y"
                    ;;
                alinux)
                    ...
                    ;;
                suse | opensuse*)
                    install_packages "zypper" "zypper install -y"
                    ;;
                void)
                    install_packages "xbps" "xbps-install -Sy"
                    ;;
                *)
...
install_packages() {
    ...
    apt)
        ...
        ;;
    dnf)
        $usesudo $install_command epel-release
    ...
    apk)
        ...
        ;;
    pkg)
        $usesudo $package_manager update
...
install_speedtest() {
    if [ "$(uname)" == "Darwin" ]; then
        ...
        12.*)
            $usesudo pkg add "https://install.speedtest.net/app/cli/ookla-speedtest-1.2.0-freebsd12-x86_64.pkg"
            ;;
        13.*)
            $usesudo pkg add "https://install.speedtest.net/app/cli/ookla-speedtest-1.2.0-freebsd13-x86_64.pkg"
            ;;
        *) return 1 ;;
        esac
    else
...
check_Net() {
    ...
    if [[ mode_output -eq 1 ]]; then
        case "$outputfile" in
        *.[aA][nN][sS][iI])
            echo "$net_report" >>"$outputfile" 2>/dev/null
            ;;
        *.[jJ][sS][oO][nN])
            echo "$netdata" >>"$outputfile" 2>/dev/null
            ;;
